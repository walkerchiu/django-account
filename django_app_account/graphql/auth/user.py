...
from django_app_account.graphql.auth.types.user import UserNode
from django_app_account.models import User
from django_app_account.variables.protected_email import PROTECTED_EMAIL
...
class UpdateEmail(graphene.relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, root, info: ResolveInfo, **input):
        ...
        if DJANGO_APP_TENANT_INSTALLED and user.email != email_original:
...
class UpdateUser(graphene.relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, root, info: ResolveInfo, **input):
        ...
        if DJANGO_APP_TENANT_INSTALLED and user.is_owner and user.email != old_email:
            with schema_context(settings.PUBLIC_SCHEMA_NAME):
                tenant_service = TenantService()
                tenant_service.updateEmail(
